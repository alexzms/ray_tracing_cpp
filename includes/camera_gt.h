...
class camera_gt {
  ...
  private:
    ...
    ray get_ray(int i, int j) const {
        ...
        auto ray_time = utilities::random_double();
    ...
    vec3 pixel_sample_square() const {
        ...
        auto px = -0.5 + utilities::random_double();
        auto py = -0.5 + utilities::random_double();
    ...
    color ray_color(const ray& r, int depth, const hittable& world) const {
        ...
        if (!world.hit(r, interval(0.001, utilities::infinity), rec))
            return background;

        ray scattered;
        color attenuation;
//        color color_from_emission = rec.surface_material->emitted(rec.u, rec.v, rec.p);
//
